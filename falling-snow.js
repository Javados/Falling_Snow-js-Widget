...
$(document).ready(function() {
    ...
    try {
        ...
        document.getElementById('shape_tech').style.display = 'none';
    }

    // js for open/close support-form box
    $('.feedback-box').find('.btn-green').on('click', function() {
        $('.support-form').slideToggle();
    });
    // closed support-form when click not this
    $(document).mouseup(function (e){
        var thisBox = $('.support-form');
        var thisBtn = $('.feedback-box').find('.btn-green');
        if (!thisBox.is(e.target) && thisBox.has(e.target).length === 0 
            && !thisBtn.is(e.target) && thisBtn.has(e.target).length === 0) {
			thisBox.slideUp();
		}
	});

    // js for support form validation
    // clear labels error
    $('#email_send_home').find('#name').on('focus', function() {
        $(this).siblings('label.error').remove();
    });
    $('#email_send_home').find('#email').on('focus', function() {
        $(this).siblings('label.error').remove();
    });
    $('#email_send_home').find('#textarea').on('focus', function() {
        $(this).siblings('label.error').remove();
    });
    // Check send form
    $('#email_send_home').on('submit', function(etg) {
        etg.preventDefault();
        var nameInput = $(this).find('#name');
        var emailInput = $(this).find('#email');
        var textarreaInput = $(this).find('#textarea');
        var emptyDetect = /[^\s]/g;
        var emptyError = nameInput.val().match(emptyDetect);
        var detectInputName = /[\w\dа-я]{2,}/gi;
        var validInputName = nameInput.val().match(detectInputName);
        var detectNumbName = /[^\d]/gi;
        var validNumbName = nameInput.val().match(detectNumbName);
        var detectEmail = /[\w,\d,.]+@[\da-z]+(\.)[\da-z]+/gi;
        var validEmail = emailInput.val().match(detectEmail);
        var validTextarrea = textarreaInput.val().match(emptyDetect);
        if (emptyError) {
            if (validInputName) {
                if (validNumbName) {
                    ...
                    if (validEmail) {
                        ...
                        if (validTextarrea) {
                            textarreaInput.addClass('valid');
                            $.ajax({
                                type: "POST",
                                url: "mail.php",
                                data: $('#email_send_home').serialize()
                            }).done(function() {
                                $('#email_send_home').find('.succes-formsubmit').addClass('formsubmitsucces');
                                $('#email_send_home').find('.btn-yellow').attr('disabled', true);
                                setTimeout(function() {
                                    $('#email_send_home').find('.succes-formsubmit').removeClass('formsubmitsucces');
                                    $('#email_send_home').find('.form-support-input').children('.valid').removeClass('valid'); 
                                    $('#email_send_home').trigger("reset");
                                    $('#email_send_home').find('.form-support-input').children('input').removeClass('error');
                                    $('#email_send_home').find('.btn-yellow').attr('disabled', false);
                                },4000);
                            });
                        }
                        else {
                            textarreaInput.addClass('error');
                            textarreaInput.after('<label class="lab-err error">Напишите сообщение!</label>');
                        }
                    }
                    else {
                        emailInput.addClass('error');
                        emailInput.after('<label class="error">Неправильный адрес!</label>');
                    }
                }
                else {
                    nameInput.addClass('error');
                    nameInput.after('<label class="error">Имя не может быть числом!</label>');
                }
            }
            else {
                nameInput.addClass('error');
                nameInput.removeClass('valid');
                nameInput.after('<label class="error">Минимум 2 символа!</label>');  
            }
        }
        else {
            nameInput.addClass('error');
            nameInput.removeClass('valid');
            nameInput.after('<label class="error">Укажите ваше имя!</label>');
        }

    });
    // check validation for info-icons
    $('#email_send_home').find('#name').on('keyup', function(etg) {
        var emptyDetect = /[^\s]/g;
        var emptyError = $(this).val().match(emptyDetect);
        var detectInputName = /[\w\dа-я]{2,}/gi;
        var validInputName = $(this).val().match(detectInputName);
        var detectNumbName = /[^\d]/gi;
        var validNumbName = $(this).val().match(detectNumbName);
        if (emptyError) {
            if (validInputName) {
                if (validNumbName) {
                    $(this).addClass('valid');
    ...
    $('#email_send_home').find('#name').on('blur', function(etg) {
        if ($(this).val() === "") {
            $(this).removeClass('error');
        }
    });
    $('#email_send_home').find('#email').on('blur', function(etg) {
        if ($(this).val() === "") {
            $(this).removeClass('error');
        }
    });
    $('#email_send_home').find('#textarea').on('blur', function(etg) {
        if ($(this).val() === "") {
            $(this).removeClass('error');
        }
    });

    $('#email_send_home').find('#email').on('keyup', function(etg) {
        var detectEmail = /[\w,\d,.]+@[\da-z]+(\.)[\da-z]+/gi;
    ...
    $('#email_send_home').find('#textarea').on('keyup', function(etg) {
        var emptyDetect = /[^\s]/g;
        var validTextarrea = $(this).val().match(emptyDetect);
        if (validTextarrea) {
            $(this).addClass('valid');
        }
        else {
